...
{
    int nAvailSegs=(iDivUp(w,BLCK_SIZE)*2)*(iDivUp(h,BLCK_SIZE)*2);
	...
	InitCUDA(width,height,nSegments,LAB_SLIC);
...
void FastImgSeg::DoSegmentation(SEGMETHOD eMethod, double weight)
{
    ...
    cv::Mat t=cv::Mat(height,width,CV_32SC1,segMask,width*sizeof(int));
