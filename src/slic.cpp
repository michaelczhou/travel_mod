	...
	const int&					NUMITR)
{
	...
	double invxywt = 2.0/(STEP*STEP);//NOTE: this is different from how usual SLIC/LKM works
...
void SLIC::PerformSLICO_ForGivenK(
	...
	const int&				    numItr)// number of iteration
{
	vector<double> kseedsl(0);
	vector<double> kseedsa(0);
	vector<double> kseedsb(0);
	...
	PerformSuperpixelSegmentation_VariableSandM(kseedsl,kseedsa,kseedsb,kseedsx,kseedsy,klabels,STEP,numItr); //NOTE
	numlabels = kseedsl.size();
